...
#define   TIMER_ID_DELIVER_FRAME 1001

#include "OpencvConstant.h"
...
private:
    CInteractAnchorDlg* m_pDialog;
public:
    QHVCInteractLocalVideoRenderCallback(CInteractAnchorDlg* pDialog) {
        m_pDialog = pDialog;
    }
...
private:
    CInteractAnchorDlg* m_pDialog;
public:
    QHVCInteractRemoteVideoRenderCallback(CInteractAnchorDlg* pDialog) {
        m_pDialog = pDialog;
    }

    virtual bool OnRenderVideoFrame(const char* userId, QHVC::INTERACT::VideoFrame& videoFrame)
    {
        if (strcmp(m_pDialog->m_strFirstRemoteUserId.c_str(), userId)) {
            return true;
        }
...
private:
    CInteractAnchorDlg* m_pDialog;
public:
    QHVCInteractAudioFrameCallback(CInteractAnchorDlg* pDialog) {
        m_pDialog = pDialog;
    }

    virtual bool OnAudioFrame(QHVC::INTERACT::AudioFrame& audioFrame)
    {
        return true;
    }

    virtual bool OnLocalAudioFrame(QHVC::INTERACT::AudioFrame& audioFrame)
    {
        return true;
    }
};
...
, m_bEngineLoadOk(false)
{
    ...
    m_bUseExtendRender = DebugSetting::GetInstance().IsUseExtRender();
    m_bUseExtendCapture = DebugSetting::GetInstance().IsUseExtCapture();
    m_pLocalVideoRenderCallback = NULL;
    m_pRemoteVideoRenderCallback = NULL;
    m_pAudioFrameCallback = NULL;
}
...
CInteractAnchorDlg::~CInteractAnchorDlg()
{
    ...
    if (m_pLocalVideoRenderCallback) {
        delete m_pLocalVideoRenderCallback;
        m_pLocalVideoRenderCallback = NULL;
    }

    if (m_pRemoteVideoRenderCallback) {
        delete m_pRemoteVideoRenderCallback;
        m_pRemoteVideoRenderCallback = NULL;
    }

    if (m_pAudioFrameCallback) {
        delete m_pAudioFrameCallback;
        m_pAudioFrameCallback = NULL;
    }
}
...
{
    if (m_bUseExtendRender) {
        m_pLocalVideoRenderCallback = new QHVCInteractLocalVideoRenderCallback(this);
        m_pRemoteVideoRenderCallback = new QHVCInteractRemoteVideoRenderCallback(this);

    }

    m_pAudioFrameCallback = new QHVCInteractAudioFrameCallback(this);
    ...
    //创建一个名字为LocalWindow的窗口
    cv::namedWindow("LocalWindow", CV_WINDOW_NORMAL);
    cv::resizeWindow("LocalWindow", 400, 400);
    cv::namedWindow("RemoteWindow", CV_WINDOW_NORMAL);
    cv::resizeWindow("RemoteWindow", 200, 200);

    if (m_bUseExtendCapture) {
        SetTimer(GetHWND(), TIMER_ID_DELIVER_FRAME, 200, NULL);
    }
...
void CInteractAnchorDlg::OnServerApiJoinRoomResult(const InteractServerApiRet& ret, const InteractRoomModel& interactRoomModel)
{
    ...
    if (ret.errCode == 0) {
        ...
        if (m_bUseExtendRender) {
            QHVC::INTERACT::QHVCInteract::SetLocalVideoRenderCallback(m_pLocalVideoRenderCallback);
            QHVC::INTERACT::QHVCInteract::SetRemoteVideoRenderCallback(m_pRemoteVideoRenderCallback);
        }
        QHVC::INTERACT::QHVCInteract::SetAudioFrameCallback(m_pAudioFrameCallback);

        if (m_bUseExtendCapture) {
            QHVC::INTERACT::QHVCInteract::OpenCollectingData();
        }
        ...
        tstring vendor = DebugSetting::GetInstance().GetVendor();
        if (vendor.size() > 0) {
            jobj["vendor"] = STR2A(vendor);
        }
...
LRESULT CInteractAnchorDlg::OnChangeClientRoleSuccess(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    ...
    if (clientRole == QHVC::INTERACT::CLIENT_ROLE_BROADCASTER) {
        ...
        // 注意：停止预览需要SetupLocalVideo为空，否则刷新可能有问题, 在UpdateVideoList中完成
...
LRESULT CInteractAnchorDlg::OnAudioMixingFinished(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    ...
    if (m_pInfoPanel) {
        m_pInfoPanel->OnAudioMixingFinish();
    }
...
LRESULT CInteractAnchorDlg::OnTimer(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    ...
    if (wParam == TIMER_ID_HEART && m_bJoinChannelSuccess) {
    ...
    else if (wParam == TIMER_ID_DELIVER_FRAME) {
        static int img_index = 0;
        img_index++;
        std::string img_path = "D:\\Image\\Image_4.jpg";
        if (img_index % 2 == 0) {
            img_path = "D:\\Image\\Image_5.jpg";
        }

        cv::Mat img = cv::imread(img_path.c_str(), CV_LOAD_IMAGE_UNCHANGED);
        if (img.empty()) {
            return 0;
        }
        cv::Mat yuvImg;
        ...
        QHVC::INTERACT::VideoFrame videoFrame;
        memset(&videoFrame, sizeof(videoFrame), 0);
        videoFrame.width = width;
        videoFrame.height = height;
        videoFrame.yBuffer = yuvImg.data;
        videoFrame.yStride = width;
        videoFrame.uBuffer = yuvImg.data + width * height;
        videoFrame.uStride = width / 2;
        videoFrame.vBuffer = yuvImg.data + width * height * 5 / 4;
        videoFrame.vStride = width / 2;
        videoFrame.rotation = 0;
        videoFrame.type = QHVC::INTERACT::FRAME_TYPE_YUV420;
        videoFrame.renderTimeMs = GetTickCount();

        QHVC::INTERACT::QHVCInteract::IncomingCollectingCapturedData(&videoFrame);
    }
...
void CInteractAnchorDlg::UpdateMixStreamInfo()
{
    ...
    for (int i = 0; i < (int)m_vectVideoUId.size(); i++) {
        ...
        if (i == 0) {
            ...
            pMixStreamRegion[i].y = DebugSetting::GetInstance().GetMixIntValue(_T("region_y3"), 215);
...
void CInteractAnchorDlg::UpdateVideoList()
{
    ...
    for (int i = 0; i < (int)m_vectVideoWnd.size(); i++) {
        ...
        if ((int)m_vectUIdTempErase.size() > 0 || (int)m_vectUIdTempAdd.size() > 0) {
                ...
                if (pControl != NULL && pControl->GetHWND() != NULL && ::IsWindow(pControl->GetHWND())) {
                    if (strcmp(myUserId.c_str(), m_vectVideoUId[i].c_str()) == 0) {
                        ...
                        if (!m_bUseExtendRender) {
                            ...
                            HWND hWnd = (HWND)cvGetWindowHandle("LocalWindow");
                            ::SetParent(hWnd, pControl->GetHWND());
                        ...
                        if (!m_bUseExtendRender) {
                            ...
                            if (m_strFirstRemoteUserId.size() == 0) {
                                m_strFirstRemoteUserId = m_vectVideoUId[i];
                            }

                            QHVC::INTERACT::QHVCInteract::SetRemoteVideoExternalRender(m_vectVideoUId[i].c_str(), "");

                            if (strcmp(m_strFirstRemoteUserId.c_str(), m_vectVideoUId[i].c_str()) == 0) {
                                HWND hWnd = (HWND)cvGetWindowHandle("RemoteWindow");
                                ::SetParent(hWnd, pControl->GetHWND());
                                ::ShowWindow(hWnd, SW_SHOW);
                            }
    ...
    if ((int)m_vectUIdTempAdd.size() > 0 || (int)m_vectUIdTempErase.size() > 0) {
        // 远端视频有数据了，更新合流信息
        UpdateMixStreamInfo();
    }

    for (int i = 0; i < (int)m_vectUIdTempErase.size(); i++) {
        std::string myUserId = STR2A(InteractGlobalManager::GetInstance().GetInteractUserId());
        std::string uid = m_vectUIdTempErase[i];
        if (strcmp(myUserId.c_str(), uid.c_str()) != 0) {
            if (m_bUseExtendRender) {
                QHVC::INTERACT::QHVCInteract::RemoveRemoteVideo("", uid.c_str());
            }
            else {
                if (strcmp(m_strFirstRemoteUserId.c_str(), uid.c_str()) == 0) {
                    HWND hWnd = (HWND)cvGetWindowHandle("RemoteWindow");
                    ::ShowWindow(hWnd, SW_HIDE);
                    m_strFirstRemoteUserId = "";
                }
            }
        }
    ...
    m_vectUIdTempAdd.clear();
    m_vectUIdTempErase.clear();
...
void CInteractAnchorDlg::AddUIdToList(const std::string& uid)
{
    ...
    m_vectUIdTempAdd.push_back(uid);
}
...
void CInteractAnchorDlg::RemoveUIdFromList(const std::string& uid)
{
    for (int i = 0; i < (int)m_vectVideoUId.size(); i++) {
        if (strcmp(uid.c_str(), m_vectVideoUId[i].c_str()) == 0) {
            ...
            m_vectUIdTempErase.push_back(uid);
...
void CInteractAnchorDlg::RebuildUIdList(const std::vector<InteractUserModel>& vectUserModel)
{
    ...
    for (std::vector<std::string>::iterator it = m_vectVideoUId.begin(); it != m_vectVideoUId.end();) {
        ...
        if (itFind == vectNewUid.end()) {
            m_vectUIdTempErase.push_back(*it);
...
void CInteractAnchorDlg::AddLog(const tstring & logInfo)
{
    ...
    CListUI* m_pListLog = static_cast<CListUI*>(FindControl(_T("list_log")));
    if (m_pListLog != NULL) {
        while (m_pListLog->GetCount() < (int)m_vectLog.size())
        {
...
LRESULT CInteractAnchorDlg::OnToolbarShow(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    int tag = (int)wParam;
    if (!IsAudioMode()) {
        if (tag >= 0 && tag < (int)m_vectVideoWnd.size()) {
...
{
    int tag = (int)wParam;
    if (tag >= 0 && tag < (int)m_vectVideoUId.size()) {

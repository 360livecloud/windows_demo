...
CInteractInfoPanel::CInteractInfoPanel()
{
    ...
    m_bWindowCapture = false;
    m_bAudioMixing = false;
}
...
void CInteractInfoPanel::OnClick(TNotifyUI& msg)
{
    ...
    else if (sCtrlName == _T("btn_capture_screen")) {
        if (!m_bScreenCapture) {
            CInteractScreenCaptureDlg dlg;
            if (dlg.DoModal(::GetParent(GetHWND())) == IDOK) {
                if (m_bWindowCapture) {
                    QHVC::INTERACT::QHVCInteract::StopScreenCapture();
                    m_bWindowCapture = false;
                    CButtonUI * pBtnCaptureScreen = static_cast<CButtonUI*>(FindControl(_T("btn_capture_window")));
                    if (pBtnCaptureScreen) {
                        pBtnCaptureScreen->SetText(_T("直播窗口"));
                    }
                }

                static int BITRATE = 0;
                RECT* rect = dlg.GetSelectRect();
                int ret = QHVC::INTERACT::QHVCInteract::StartScreenCapture(NULL, 15, rect, BITRATE);
                if (ret == QHVC::INTERACT::ERR_OK) {
                    m_bScreenCapture = true;
                    msg.pSender->SetText(_T("屏幕直播中..."));
                } if (ret == QHVC::INTERACT::ERR_NOT_SUPPORTED) {
                    DuiMessageBox(::GetParent(GetHWND()), _T("暂不支持屏幕直播"), INTERACT_TITLE, MB_YESNO);
                }
            }
        }
        else {
            QHVC::INTERACT::QHVCInteract::StopScreenCapture();
            m_bScreenCapture = false;
            msg.pSender->SetText(_T("直播屏幕"));
        }

    }
    else if (sCtrlName == _T("btn_capture_window")) {
        if (!m_bWindowCapture) {
            CInteractWndSelectDlg dlg;
            if (dlg.DoModal(::GetParent(GetHWND())) == IDOK) {
                if (m_bScreenCapture) {
                    QHVC::INTERACT::QHVCInteract::StopScreenCapture();
                    m_bScreenCapture = false;
                    CButtonUI * pBtnCaptureScreen = static_cast<CButtonUI*>(FindControl(_T("btn_capture_screen")));
                    if (pBtnCaptureScreen) {
                        pBtnCaptureScreen->SetText(_T("直播屏幕"));
                    }
                }

                static int BITRATE = 0;
                HWND hwnd = dlg.GetSeletcWnd();
                int ret = QHVC::INTERACT::QHVCInteract::StartScreenCapture(hwnd, 15, NULL, BITRATE);
                if (ret == QHVC::INTERACT::ERR_OK) {
                    m_bWindowCapture = true;
                    msg.pSender->SetText(_T("窗口直播中..."));
                }
                else if (ret == QHVC::INTERACT::ERR_NOT_SUPPORTED) {
                    DuiMessageBox(::GetParent(GetHWND()), _T("暂不支持窗口直播"), INTERACT_TITLE, MB_YESNO);
                }
            ...
            QHVC::INTERACT::QHVCInteract::StopScreenCapture();
            m_bWindowCapture = false;
            msg.pSender->SetText(_T("直播窗口"));
        }
    }
    else if (sCtrlName == _T("btn_test_mixing")) {
        if (!m_bAudioMixing) {
            CInteractAudioMixingDlg dlg;
            if (dlg.DoModal(::GetParent(GetHWND())) == IDOK) {
                const CInteractAudioMixingDlg::Config* pConfig = dlg.GetConfig();
                int ret = QHVC::INTERACT::QHVCInteract::StartAudioMixing(pConfig->filePath.c_str(), pConfig->loopback, pConfig->replace, pConfig->cycle);
                if (ret == QHVC::INTERACT::ERR_OK) {
                    m_bAudioMixing = true;
                    msg.pSender->SetText(_T("混音中..."));
                }
                else if (ret == QHVC::INTERACT::ERR_NOT_SUPPORTED) {
                    DuiMessageBox(::GetParent(GetHWND()), _T("暂不支持混音"), INTERACT_TITLE, MB_YESNO);
                }
            }
        }
        else {
...
void CInteractInfoPanel::OnItemLink(TNotifyUI& msg)
{
    ...
    if (pListTextElement != NULL) {
        ...
        if (msg.wParam == 0) {
            ...
            if (tag >= 0 && tag < (int)m_vectAnchor.size()) {
...
void CInteractInfoPanel::OnItemSelect(TNotifyUI& msg)
{
    ...
    if (sCtrlName == _T("cmb_video_list") || sCtrlName == _T("cmb_audio_list") || sCtrlName == _T("cmb_micro_list")) {
        ...
        if (sCtrlName == _T("cmb_video_list")) {
            ...
            if (nIdx >= 0 && nIdx < (int)m_vectMicroDevicInfo.size()) {
...
void CInteractInfoPanel::FillComboList(const std::string& comboName, std::vector<InteractDeviceInfo> vectDeviceInfo, const std::string& deviceId)
{
    ...
    if (pCombo != NULL) {
        pCombo->RemoveAll();
        int selectIdx = -1;
        for (int i = 0; i < (int)vectDeviceInfo.size(); i++) {
...
LPCTSTR CInteractInfoPanel::GetItemText(CControlUI* pList, int iItem, int iSubItem)
{
    ...
    if (pList->GetParent()->GetParent()->GetName() == _T("list_user")) {
        ...
        if (iItem < (int)m_vectDebugInfo.size()) {
            switch (iSubItem)
            {
                case 0: {
...
void CInteractInfoPanel::SetAnchorList(const std::vector<InteractUserModel>& vectAnchor)
{
    ...
    for (int i = 0; i < (int)m_vectAnchor.size(); i++)
    {
        if (m_vectAnchor[i].identity == InteractConstant::USER_IDENTITY_ANCHOR) {
            ...
            std::string duration = Util::Time::format_hhmmss((int)(time_current - time_begin));
    ...
    if (mUserIdentity != userIdentity || !m_bInitControlStatus) {
        ...
        CButtonUI * pBtnCaptureScreen = static_cast<CButtonUI*>(FindControl(_T("btn_capture_screen")));
        CButtonUI * pBtnCaptureWindow = static_cast<CButtonUI*>(FindControl(_T("btn_capture_window")));
        CButtonUI * pBtnTestMixing = static_cast<CButtonUI*>(FindControl(_T("btn_test_mixing")));
        ...
        if (userIdentity == InteractConstant::USER_IDENTITY_ANCHOR || userIdentity == InteractConstant::USER_IDENTITY_GUEST) {
            ...
            if (pBtnCaptureScreen) {
                pBtnCaptureScreen->SetVisible(true);
            }
            if (pBtnCaptureWindow) {
                pBtnCaptureWindow->SetVisible(true);
            }
            if (pBtnTestMixing) {
                pBtnTestMixing->SetVisible(true);
            }
        }
        else {
            if (pLayoutVideo) {
            ...
            if (pBtnCaptureScreen) {
                pBtnCaptureScreen->SetVisible(false);
            }
            if (pBtnCaptureWindow) {
                pBtnCaptureWindow->SetVisible(false);
            }
            if (pBtnTestMixing) {
                pBtnTestMixing->SetVisible(false);
            }
        }
        InitDevicInfo();
    }
...
void CInteractInfoPanel::SetInteractStats(const InteractStats& mInteractStats)
{
    ...
    CListUI* pRoomInfoList = static_cast<CListUI*>(FindControl(_T("list_room")));
    if (pRoomInfoList) {
        if (pRoomInfoList->GetCount() > (int)m_vectDebugInfo.size()) {
            pRoomInfoList->RemoveAll();
        }
        while (pRoomInfoList->GetCount() < (int)m_vectDebugInfo.size()) {
            {
                CListTextElementUI* pListElement = new CListTextElementUI;
...
void CInteractInfoPanel::OnAudioMixingFinish()
{
    m_bAudioMixing = false;
    CButtonUI * pBtnTestMixing = static_cast<CButtonUI*>(FindControl(_T("btn_test_mixing")));
    if (pBtnTestMixing) {
        pBtnTestMixing->SetText(_T("混音测试"));
    }
}